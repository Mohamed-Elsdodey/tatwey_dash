...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('rows.update',$row->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
        ...
        <div class=" row my-4">
            @foreach($categories as $category)
                <div class="col-md-4 p-1">
                    <span class="form-check form-switch  " @if( app()->getLocale()=='en') style="border:1px solid #F3F3F9;padding: 10px; padding-left: 40px;border-radius: 4px;" @else  style="border:1px solid #F3F3F9;padding: 10px; padding-right: 40px;border-radius: 4px;" @endif>
                      <input class="form-check-input  checkbox" @foreach($row_categories_id as $id) @if($category->id==$id) checked @endif @endforeach type="checkbox" name="categories[]" value="{{$category->id}}"
